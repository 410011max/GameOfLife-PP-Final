...
__global__ void simpleLifeKernel(int width, int height, const uint8_t* grid, uint8_t* new_grid) {
...
__global__ void bitLifeKernel(int width, int height, uint8_t* grid, uint8_t* new_grid) {
...
__global__ void kernelUpdate(int width, int height, uint8_t* grid, uint8_t* new_grid) {
    int x = blockIdx.x * blockDim.x + threadIdx.x + 1;
    int y = blockIdx.y * blockDim.y + threadIdx.y + 1;
...
void Life_cuda::update()
{
    ...
    // simpleLifeKernel<<<gridDim, blockDim>>>(width, height, d_grid, d_new_grid);
    // bitLifeKernel<<<gridDim, blockDim>>>(width, height, d_grid, d_new_grid);
